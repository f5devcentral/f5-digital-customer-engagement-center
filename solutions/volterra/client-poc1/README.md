...
| vpcShared | terraform-aws-modules/vpc/aws | ~> 2.0 |
...
| sharedVPCs | The shared VPCs with common services like security, firewall, load balancing, and more. | <pre>map(object({<br>    cidr            = string<br>    public_subnets  = list(any)<br>    private_subnets = list(any)<br>  }))</pre> | <pre>{<br>  "hub": {<br>    "cidr": "100.64.0.0/20",<br>    "private_subnets": [<br>      "100.64.3.0/24",<br>      "100.64.4.0/24",<br>      "100.64.5.0/24"<br>    ],<br>    "public_subnets": [<br>      "100.64.0.0/24",<br>      "100.64.1.0/24",<br>      "100.64.2.0/24"<br>    ]<br>  }<br>}</pre> | no |
