...
#resource "aws_lambda_function" "UserPoolDomainSetterLambda" {
#  filename      = "aws-waf-dashboard/cloudformation-custom-resources/domain-setter-lambda.zip"
#  function_name = "${var.projectPrefix}-UserPoolDomainSetterLambda-${random_id.buildSuffix.hex}"
#  role          = aws_iam_role.UserPoolDomainSetterLambdaRole.arn
#  handler       = "lambda_function.handler"
#  runtime       = "python3.7"
#  memory_size   = 128
#  timeout       = 60
#
#  source_code_hash = filebase64sha256("aws-waf-dashboard/cloudformation-custom-resources/domain-setter-lambda.zip")
#}

#resource "aws_lambda_function" "ESCognitoAuthSetterLambda" {
#  filename      = "aws-waf-dashboard/cloudformation-custom-resources/es-cognito-auth-lambda.zip"
#  function_name = "${var.projectPrefix}-ESCognitoAuthSetterLambda-${random_id.buildSuffix.hex}"
#  role          = aws_iam_role.ESCognitoAuthSetterLambdaRole.arn
#  handler       = "lambda_function.handler"
#  runtime       = "python3.7"
#  memory_size   = 128
#  timeout       = 900
#
#  source_code_hash = filebase64sha256("aws-waf-dashboard/cloudformation-custom-resources/es-cognito-auth-lambda.zip")
#}

resource "aws_lambda_function" "KibanaCustomizerLambda" {
  filename      = "kibana-customizer-lambda.zip"
  ...
  source_code_hash = filebase64sha256("kibana-customizer-lambda.zip")
  environment {
    variables = {
      REGION = "us-east-1"
